exports.plugin = {
    ...
    async register (app, options) {
        ...
        const {Crypto, Identities} = require("@arkecosystem/crypto"), {Slots} = Crypto;
            ...
            if (secrets && Array.isArray(secrets)) {
                for (const secret of secrets) {
                    try {
                        ...
                        if (publicKey) {
                            const delegateName = database.walletManager.findByPublicKey(publicKey).username;
        ...
        processNextSlot = async () => {
            ...
            if (!round.order.length) {
                ...
                const thisRound = await database.getActiveDelegates(round.calculated);
            ...
            if (delegates.monitoring.length) {
                ...
                for (const delegate of delegates.monitoring) {
                    ...
                    if (position > -1) {
                        const delegateInfo = await database.delegates.findById(publicKey),
